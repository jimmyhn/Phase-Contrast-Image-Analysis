...
conversion4x = 1.6125 # microns/pixels
conversion10x = 0.645
...
for a in types_to_analyze:
   ...
   for f, folder in enumerate(folders_to_analyze): # first for loop goes through each sample
      ...
      for image_index, image in enumerate(analysisReference): # goes through each image
         ...
         # Rotation Process (if horizontal type):
         ...
         if bool(re.search(r"4x",image['name'],re.IGNORECASE)):
            ...
            for quadrant_index, quadrant in enumerate(quadrants): # loops through each quadrant: Q1,Q2,Q3,Q4 image
               ...
               if not isMaskStored: #temporary fix for the wrong mask rotation issue
               ...
               for unique_value in (unique_mask_values): # loops through each cell
                  ...
                  end_index = np.searchsorted(unique_cell_pixels,65535,side='right') # 65535 is the max value which is yellow
                  ...
                  if unwanted_to_total < filter_threshold: 
                     ...
                     conversion =  conversion4x
                     if region.perimeter == 0 or region.axis_minor_length == 0:
                        circularity = np.nan
                        polarity = np.nan
                     else:
                        print(f"The conversion factor is now:{conversion}microns/pixel")
                        length = region.axis_major_length * conversion
                        width = region.axis_minor_length * conversion
                        perimeter = region.perimeter * conversion
                        area = region.area * (conversion ** 2)

                        circularity = ((2*np.sqrt(np.pi*area))/perimeter)
                        polarity = (length/width)
                     ...
                     raw_data.append({
                        ...
                        f'Major Length({micro}m):': length, 
                        f'Minor Length({micro}m):': width, 
                        f'Perimeter({micro}m):': perimeter, 
                        f'Area({micro}m{squared}):': area
                     })
                     ...
                     fake_cells += 1
                     cells_to_remove_P2.append(unique_value) # used for other outputs including outliers and visual
            ...
            normal_distribution_data, _ , cells_to_remove_P3, quadrants = remove_outliers_IQR(raw_data_for_P3mask)
            ...
            if os.path.isdir(os.path.join(os.getcwd(),"Unprocessed_Masks")):
               ...
               mask_files = unprocessed_masks[0]['Filename']
               isMaskStored = False #boolean setup to check if mask is stored or not
               if len(mask_files) > 0:
            ...
            if not isMaskStored: # can be used to fix the unprocessed mask but make sure keep track/run through on whole sample
            ...
            for unique_value in (unique_mask_values): # loops through each cell
               ...
               if unwanted_to_total < filter_threshold: 
                  ...
                  match b:
                     case '10x':
                        conversion = conversion10x
                     case '20x':
                        conversion = conversion20x
                  ...
                  print(f"Orientation: {np.degrees(region.orientation)}\nCircularity: {circularity}\nPolarity: {polarity}\nLength(px): {region.axis_major_length} & Length({micro}m): {length}\nWidth(px): {region.axis_minor_length} & Width({micro}m): {width}\nPerimeter(px): {region.perimeter} & Perimeter({micro}m): {perimeter}\nArea(px^2): {region.area} & Area({micro}m^2): {area}\n ")
                  # Raw data collection
                  raw_data.append({
                     'Orientation(deg):': np.degrees(region.orientation),
                     'Circularity:': circularity,
